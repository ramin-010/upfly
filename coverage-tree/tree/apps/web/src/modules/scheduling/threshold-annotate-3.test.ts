...
import { summarise, withDefaults } from './threshold-annotate-3.js';

describe('threshold-annotate-3', () => {
  it('partitions a draft allocation without losing the rest', () => {
  ...
  it('validates a partial allocation without losing the rest', () => {
    const rows = [withDefaults({ id: 'a1' }), withDefaults({ id: 'b1' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('normalises a settled allocation without losing the rest', () => {
    const rows = [withDefaults({ id: 'a2' }), withDefaults({ id: 'b2' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('rebalances a expired allocation without losing the rest', () => {
    const rows = [withDefaults({ id: 'a3' }), withDefaults({ id: 'b3' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('defers a pending allocation without losing the rest', () => {
  ...
  it('collapses a draft allocation without losing the rest', () => {
