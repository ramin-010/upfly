...
import { summarise, withDefaults } from './entitlement-merge-3.js';

describe('entitlement-merge-3', () => {
  ...
  it('settles a expired retention without losing the rest', () => {
    const rows = [withDefaults({ id: 'a1' }), withDefaults({ id: 'b1' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('expands a pending retention without losing the rest', () => {
  ...
  it('merges a partial retention without losing the rest', () => {
