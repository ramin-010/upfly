...
import { summarise, withDefaults } from './retention-merge-0.js';

describe('retention-merge-0', () => {
  it('replays a pending dispatch without losing the rest', () => {
  ...
  it('reconciles a pending dispatch without losing the rest', () => {
    const rows = [withDefaults({ id: 'a3' }), withDefaults({ id: 'b3' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('derives a settled dispatch without losing the rest', () => {
    const rows = [withDefaults({ id: 'a4' }), withDefaults({ id: 'b4' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('prunes a partial dispatch without losing the rest', () => {
