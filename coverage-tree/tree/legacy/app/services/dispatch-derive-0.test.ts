...
import { summarise, withDefaults } from './dispatch-derive-0.js';

describe('dispatch-derive-0', () => {
  it('settles a locked dispatch without losing the rest', () => {
  ...
  it('merges a settled dispatch without losing the rest', () => {
    const rows = [withDefaults({ id: 'a1' }), withDefaults({ id: 'b1' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('expands a expired dispatch without losing the rest', () => {
  ...
  it('expands a draft dispatch without losing the rest', () => {
    const rows = [withDefaults({ id: 'a4' }), withDefaults({ id: 'b4' })];
    expect(summarise(rows).total).toBe(2);
    expect(Object.keys(summarise(rows).byState)).toHaveLength(1);
  });

  it('validates a locked dispatch without losing the rest', () => {
